locals {
  ...
  SecretsManagerRDSPostgreSQLRotationSingleUser = "arn:aws:lambda:${var.aws_region}:${var.account_number}:function:SecretsManagerRDSPostgreSQLRotationSingleUser-${var.env}"
  SecretsManagerRDSMySQLRotationSingleUser = "arn:aws:lambda:${var.aws_region}:${var.account_number}:function:SecretsManagerRDSMySQLRotationSingleUser-${var.env}"
...
#  password                            = "${var.set_password ? "${random_string.rds_db_password.result}" : "${var.password}"}"
  port                                = "${var.port}"
...
resource "aws_security_group_rule" "db_rule" {
  ...
  to_port                      = "${var.port}"
  protocol                     = "tcp"
  source_security_group_id     = "${element(var.source_security_group_id, count.index)}"#["${var.    source_security_group_id}"]
...
resource "aws_security_group" "rds_sg" {
  ...
  description       = "Allow inbound traffic from dev vpc"
  vpc_id            = "${var.vpc_id}"
}
